...
void
createshortcutsinhibitor(struct wl_listener *listener, void *data)
{
...
deck(Monitor *m)
{
	unsigned int mw, my, h, r, oe = enablegaps, ie = enablegaps, draw_borders = 1;
	int i, n = 0;
	Client *c;
...
dwl_ipc_output_printstatus_to(DwlIpcOutput *ipc_output)
{
	...
	Client *c, *focused;
	int tagmask, state, numclients, isfocused, tag;
	const char *title, *appid;
	focused = focustop(monitor);
	zdwl_ipc_output_v2_send_active(ipc_output->resource, monitor == selmon);

	for (tag = 0 ; tag < TAGCOUNT; tag++) {
		numclients = state = isfocused = 0;
		tagmask = 1 << tag;
		...
		zdwl_ipc_output_v2_send_tag(ipc_output->resource, tag, state, numclients, isfocused);
	}
...
keypress(struct wl_listener *listener, void *data)
{
	...
	for (int i = 0; i < 2; i++)
		group->keysyms[i] = keymap_get_one_sym_by_level(en_keymap, keycode, 0, i);
	group->mods = wlr_keyboard_get_modifiers(kb);
...
monocle(Monitor *m)
{
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (!monoclegaps || !enablegaps)
			...
			resize(c, (struct wlr_box){.x = m->w.x + m->gappoh*oe, .y = m->w.y + m->gappov*oe,
				.width = m->w.width - 2 * m->gappoh*oe, .height = m->w.height - 2 * m->gappov*oe}, 0, !smartborders);
	}
	if (n)
